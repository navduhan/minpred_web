...
import { Bookmark, Check, ChevronDown, Copy, Database, FileUp, Loader2, Play, RotateCcw, ShieldCheck } from 'lucide-react';
...
function RoundedSelect({ ariaLabel, value, options, onChange, dropUp = false }: { ariaLabel: string; value: string; options: readonly SelectOption[]; onChange: (value: string) => void; dropUp?: boolean }) {
  ...
  return <div ref={root} className="relative mt-2"><button type="button" aria-label={ariaLabel} aria-haspopup="listbox" aria-expanded={open} onClick={()=>setOpen(current=>!current)} className="field flex min-h-12 items-center justify-between gap-4 text-left font-semibold"><span>{selected}</span><ChevronDown className={`h-4 w-4 shrink-0 text-[var(--muted)] transition ${open?'rotate-180':''}`}/></button>{open&&<div role="listbox" aria-label={ariaLabel} className={`absolute z-[100] max-h-64 w-full overflow-auto rounded-2xl border border-[var(--line)] bg-white p-1.5 shadow-[0_18px_45px_rgba(34,43,49,.18)] ${dropUp?'bottom-full mb-2':'top-full mt-2'}`}>{options.map(([option,label])=><button key={option} type="button" role="option" aria-selected={option===value} onClick={()=>{onChange(option);setOpen(false);}} className={`flex w-full items-center justify-between rounded-xl px-3.5 py-2.5 text-left text-sm transition ${option===value?'bg-[var(--forest)] font-bold text-white':'text-[var(--ink)] hover:bg-[var(--mist)]'}`}><span>{label}</span>{option===value&&<Check className="h-4 w-4"/>}</button>)}</div>}</div>;
}
  ...
  return <div className="mx-auto max-w-5xl space-y-8 py-5 sm:py-10">
    ...
    <section className="rounded-[2rem] border border-[var(--line)] bg-white shadow-[0_24px_70px_rgba(45,42,38,.08)]">
      ...
      <div className="border-b border-[var(--line)] p-6 sm:p-8"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Step 2</p><h2 className="mt-1 font-display text-2xl font-semibold">Add sequence data</h2></div><div className="flex gap-2"><button type="button" onClick={loadExample} className="btn-secondary">Load example</button><button type="button" onClick={clear} className="btn-secondary"><RotateCcw className="h-3.5 w-3.5"/>Clear</button></div></div>
        {mode==='accession'&&<div className="mt-6 grid gap-3 sm:grid-cols-[180px_1fr_auto]"><RoundedSelect ariaLabel="Protein database" value={database} options={[["uniprot","UniProtKB"],["ncbi","NCBI Protein"]]} onChange={value=>setDatabase(value as 'uniprot'|'ncbi')}/><input className="field mt-2" value={accessions} onChange={(e)=>setAccessions(e.target.value)} placeholder="Enter one or more accessions"/><button type="button" onClick={fetchAccessions} className="btn-secondary mt-2"><Database className="h-4 w-4"/>Retrieve</button></div>}
