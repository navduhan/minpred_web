...
export default function PredictionPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-8 py-5 sm:py-10">
    ...
    <section className="overflow-hidden rounded-[2rem] border border-[var(--line)] bg-white shadow-[0_24px_70px_rgba(45,42,38,.08)]">
      ...
      <div className="p-6 sm:p-8"><div><p className="eyebrow">Step 3</p><div className="mt-1 flex flex-col justify-between gap-1 sm:flex-row sm:items-end"><h2 className="font-display text-2xl font-semibold">Choose the result</h2><p className="text-xs leading-5 text-[var(--muted)]">Each option includes the preceding phases.</p></div><div className="mt-5 grid gap-3 sm:grid-cols-2">{[['Phase1','Phase I','Enzyme or non-enzyme'],['Phase2','Phase II','Nitrogen mineralization screening'],['Phase3','Phase III','Mineralization enzyme class'],['Phase4','Phase IV','Class-specific EC assignment']].map(([value,phase,label])=><label key={value} className={`flex cursor-pointer items-center gap-4 rounded-xl border px-4 py-3 ${level===value?'border-[var(--forest)] bg-[#fbf4f5]':'border-[var(--line)]'}`}><input type="radio" name="level" value={value} checked={level===value} onChange={()=>setLevel(value)} className="h-4 w-4 accent-[#7b2d3b]"/><span><strong className="block text-sm">{phase}</strong><span className="text-xs text-[var(--muted)]">{label}</span></span></label>)}</div></div>
        {level==='Phase4'&&<fieldset className="mt-5 rounded-2xl bg-[var(--mist)] p-4"><legend className="px-1 text-sm font-bold">How should the enzyme class be chosen?</legend><div className="mt-2 grid gap-3 sm:grid-cols-2"><label className={`cursor-pointer rounded-xl border bg-white p-4 ${phase4Mode==='automatic'?'border-[var(--forest)] ring-1 ring-[var(--forest)]':'border-[var(--line)]'}`}><span className="flex items-start gap-3"><input type="radio" name="phase4-mode" checked={phase4Mode==='automatic'} onChange={()=>setPhase4Mode('automatic')} className="mt-0.5 h-4 w-4 accent-[#7b2d3b]"/><span><strong className="block text-sm">Determine automatically</strong><span className="mt-1 block text-xs leading-5 text-[var(--muted)]">Use the Phase III prediction for each sequence.</span></span></span></label><label className={`cursor-pointer rounded-xl border bg-white p-4 ${phase4Mode==='specific'?'border-[var(--forest)] ring-1 ring-[var(--forest)]':'border-[var(--line)]'}`}><span className="flex items-start gap-3"><input type="radio" name="phase4-mode" checked={phase4Mode==='specific'} onChange={()=>setPhase4Mode('specific')} className="mt-0.5 h-4 w-4 accent-[#7b2d3b]"/><span><strong className="block text-sm">Use a specific class</strong><span className="mt-1 block text-xs leading-5 text-[var(--muted)]">Apply one selected class model to all sequences.</span></span></span></label></div>{phase4Mode==='specific'&&<label htmlFor="phase4-class" className="mt-4 block text-sm font-bold">Select enzyme class<select id="phase4-class" className="field mt-2" value={enzymeClass} onChange={event=>setEnzymeClass(event.target.value as EnzymeClass)}>{enzymeClassOptions.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label>}</fieldset>}
