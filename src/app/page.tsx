...
export default function Home() {
  return <div className="pb-16">
    <section className="reveal relative min-h-[650px] overflow-hidden py-16 sm:py-24">
      ...
      <div className="absolute right-[120px] top-[255px] h-3 w-3 rounded-full bg-[var(--mineral)] shadow-[0_0_0_16px_rgba(140,116,69,.12)]" />
      <div className="relative grid gap-14 lg:grid-cols-[90px_1fr_340px] lg:items-center">
        ...
        <div className="relative hidden h-[420px] lg:block"><div className="absolute left-1/2 top-0 h-full w-px bg-[#c8cac8]" />{['DPC','NMBroto','CKSAAP'].map((label,index)=><div key={label} className="absolute left-1/2 flex -translate-x-1/2 items-center gap-4" style={{top:`${65+index*125}px`}}><span className="h-4 w-4 rounded-full border-4 border-white bg-[var(--mineral)] shadow-[0_0_0_1px_#8c7445]"/><span className="absolute left-8 whitespace-nowrap font-mono text-[10px] font-bold tracking-[.16em] text-[var(--forest)]">{label}</span></div>)}<span className="absolute bottom-1 left-1/2 -translate-x-1/2 bg-[var(--forest-dark)] px-3 py-2 font-mono text-xs font-bold text-white">EC</span></div>
      </div>
    </section>
