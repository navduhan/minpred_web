...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ sampleId, sequence, ...(await cachedStructure(jobId, sequence, 'tertiary', () => { enforceRateLimit('tertiary-structure', clientIp(req), 5, 24 * 60 * 60 * 1000); return predictTertiary(sequence); })) });
  } catch (error: unknown) {
